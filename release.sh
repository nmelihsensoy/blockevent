#!/bin/bash
## 
## Release script for 'android-blockevent' application. 
...
if [ $# -gt 1 ]; then
    ...
    [[ "$1" =~ ^v?(([0-9]{1,2})\.([0-9])(\.([0-9]))?)$ ]] && version="_${1#v}" || \
        >&2 echo "Invalid Version: '$1'. Building versionless." 
else
    echo "Building versionless." 
fi
...
for app in ${apps[@]}; do
    ...
    for arch in ${archs[@]}; do
        ...
        [[ $? -eq 0 ]] || >&2 echo "Error occured when copying: '${output_filename}'"
